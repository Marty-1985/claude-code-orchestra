...
<!--
name: startproject
description: プロジェクト開始時の標準フロー。調査→設計→計画の3フェーズを進める。

概要: Claudeがコードベースを読み、必要ならGeminiで外部調査。Agent Teamsで研究と設計を並列化し、
計画をまとめてユーザーの承認を得たら実装フェーズへ進む。

主なフェーズ:
1) 調査（コードベース理解、外部情報はGemini）
2) 設計（アーキテクチャ、トレードオフ整理）
3) 計画（実装手順と検証方法の明確化）

使用方法やタスク運用の注意点、チームメイト操作などのガイドを含む。
-->
