...
<!--
# CLIログのベストプラクティス（日本語訳）

本ドキュメントはCLIツールのログ収集・可観測性に関するベストプラクティスをまとめた調査。

主な内容:
- ログの目的（デバッグ、監査、品質向上）
- 収集する情報（コマンド、実行時間、結果、エラーなど）
- 取り扱い注意点（機密情報の除外、PIIのマスキング）
- ログ形式（JSONL など機械可読形式）
- ローテーション、保持期間、パフォーマンスへの影響
- フックやミドルウェアによる自動収集

実装ガイドとして、フック追加→実運用テスト→ドキュメント追記の順で進め、
全体で約1時間程度の実装コストと見積もっている。
-->
