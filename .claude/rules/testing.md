...
- TDD推奨
- カバレッジ目標は80%以上
- 単体テストは高速（1テスト100ms未満）

## テスト構造

### AAAパターン

Arrange / Act / Assert の構成で書く。

### 命名規則

`test_{対象}_{条件}_{期待結果}` 形式で命名する。
...
共通セットアップは `conftest.py` に置く。

## コマンド

全テスト、ファイル指定、テスト指定、カバレッジ、失敗時停止の例を示す。

## チェックリスト

正常系・エラー系・境界値のテスト、独立性、モック、速度を確認する。
-->
